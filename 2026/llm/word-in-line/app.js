...
try {
  ...
  for (const word of words) {
    if (finite(saved?.positions?.[word.id]))
      positions[word.id] = saved.positions[word.id];
  }
  if (finite(saved?.camera?.x) && finite(saved?.camera?.scale)) {
    camera = {
      x: saved.camera.x,
      scale: Math.max(0.25, Math.min(4, saved.camera.scale)),
    };
  }
} catch {
  /* The game also works without browser storage. */
}
function save() {
  try {
...
function screenX(x) {
  return board.clientWidth / 2 + camera.x + x * camera.scale;
}
...
function render() {
  const focusId = pins.contains(document.activeElement)
  ...
  if (focusId)
    pins
      .querySelector(`[data-id="${focusId}"] button`)
...
function layout() {
  ...
  const lanes = [];
  const ordered = [...pins.children].sort(
    (a, b) => positions[a.dataset.id] - positions[b.dataset.id]
  );
  for (const pin of ordered) {
    ...
    head.setAttribute(
      'aria-label',
      `${head.textContent}؛ x = ${(positions[pin.dataset.id] / 24).toFixed(2)}؛ حرکت با چپ و راست، بازگشت با Delete`
    );
  }
  // Grow the interaction area if many labels share the same point.
  board.style.minHeight = `${Math.max(480, (28 + Math.ceil(lanes.length / 2) * 49 + 32) / 0.42)}px`;
}
function place(id, x) {
  positions[id] = x;
  selected = null;
...
document.addEventListener('pointerdown', (event) => {
  ...
  drag = {
    ...
    x: word ? positions[word.dataset.id] : camera.x,
    moved: false,
    capture,
    ghost: null,
  };
...
function finish(event, cancelled = false) {
  ...
  if (cancelled) {
    ...
    if (current.kind === 'source' && inside(board, event))
      place(current.id, worldX(event.clientX));
    if (
      current.kind === 'pin' &&
      inside(document.querySelector('.sidebar'), event)
    )
      remove(current.id);
  } else {
    if (current.kind === 'source') {
      selected = selected === current.id ? null : current.id;
      active = null;
    }
    if (current.kind === 'pan' && selected)
      place(selected, worldX(event.clientX));
    else if (current.kind === 'pan') active = null;
    if (current.kind === 'pin')
      pins
        .querySelector(`[data-id="${current.id}"] button`)
        .focus({ preventScroll: true });
  }
  suppressClick = true;
  setTimeout(() => {
...
document.addEventListener('keydown', (event) => {
  ...
  if (
    ...
    pins
      .querySelector(`[data-id="${active}"] button`)
      .focus({ preventScroll: true });
    return;
  }
  ...
  if (['ArrowLeft', 'ArrowRight'].includes(event.key)) {
    event.preventDefault();
    positions[id] +=
      (event.key === 'ArrowLeft' ? -1 : 1) * (event.shiftKey ? 24 : 6);
    active = id;
    save();
    layout();
  }
...
board.addEventListener(
  ...
  (event) => {
    ...
    if (event.ctrlKey || event.metaKey)
      zoom(
        Math.exp(-event.deltaY * unit * 0.005),
        event.clientX - board.getBoundingClientRect().left
      );
    else {
      camera.x -= (event.deltaX || event.deltaY) * unit;
      save();
      layout();
    }
  },
...
document.querySelector('#zoom-out').onclick = () => zoom(0.8);
...
document.querySelector('#fit').onclick = () => {
  ...
  camera.scale = Math.max(
    0.25,
    Math.min(1, (board.clientWidth - 160) / Math.max(1, max - min))
  );
...
document.querySelector('#reset').onclick = () => {
  ...
  save();
  render();
  announce('خط پاک شد. دوباره شروع کن.');
};
