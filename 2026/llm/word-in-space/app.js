...
try {
  ...
  if (saved && typeof saved === 'object') {
    ...
    if (
      ...
      camera = {
        scale: Math.max(0.5, Math.min(4, view.scale)),
        x: view.x,
        y: view.y,
      };
    }
  ...
  /* Storage may be unavailable; the board still works. */
}
...
function tile(word, onBoard = false) {
  ...
  button.setAttribute(
    'aria-label',
    onBoard
      ? `${word.text}؛ با کلیدهای جهت جابه‌جا کن، با Delete برگردان`
      : `${word.text}؛ انتخاب برای قرار دادن روی صفحه`
  );
  return button;
}
...
function save() {
  try {
    localStorage.setItem(storageKey, JSON.stringify({ positions, camera }));
  } catch {}
}
...
function render() {
  ...
  for (const word of words) {
    ...
    if (legacy && Number.isFinite(legacy.x) && Number.isFinite(legacy.y)) {
      positions[word.id] = {
        x:
          Math.max(0, Math.min(1, legacy.x)) *
          Math.max(0, board.clientWidth - source.offsetWidth),
        y:
          Math.max(0, Math.min(1, legacy.y)) *
          Math.max(0, board.clientHeight - source.offsetHeight),
      };
...
function updateSelection() {
  board.querySelectorAll('.board-word').forEach((element) => {
    element.classList.toggle(
      'selected',
      boardSelection.has(element.dataset.id)
    );
    element.setAttribute(
      'aria-pressed',
      String(boardSelection.has(element.dataset.id))
    );
  });
}
...
function boardPoint(event) {
  ...
  return {
    x: (event.clientX - rect.left - board.clientLeft - camera.x) / camera.scale,
...
function pinchPoints() {
  ...
  const rect = board.getBoundingClientRect();
  return {
    x: (a.x + b.x) / 2 - rect.left - board.clientLeft,
    y: (a.y + b.y) / 2 - rect.top - board.clientTop,
...
function applyCamera() {
  ...
  const major = 120 * camera.scale,
    minor = 24 * camera.scale;
...
document.addEventListener('pointerdown', (event) => {
  ...
  if (source && board.contains(source)) {
    ...
    if (event.shiftKey) {
      if (boardSelection.has(id)) boardSelection.delete(id);
      else boardSelection.add(id);
...
document.addEventListener('pointermove', (event) => {
  if (
    placementTap?.pointerId === event.pointerId &&
    Math.hypot(event.clientX - placementTap.x, event.clientY - placementTap.y) >
      5
  )
    placementTap = null;
  if (touches.has(event.pointerId)) {
    ...
    if (pinch) {
      ...
      camera.scale = Math.max(
        0.5,
        Math.min(4, (pinch.scale * gesture.distance) / pinch.distance)
      );
  ...
  if (drag.kind === 'marquee') {
    ...
    if (
      !drag.moved &&
      Math.hypot(point.x - drag.start.x, point.y - drag.start.y) < 5
    )
      return;
    drag.moved = true;
    const left = Math.min(point.x, drag.start.x),
      top = Math.min(point.y, drag.start.y);
    const right = Math.max(point.x, drag.start.x),
      bottom = Math.max(point.y, drag.start.y);
    ...
    board.querySelectorAll('.board-word').forEach((element) => {
      if (
        element.offsetLeft < right &&
        element.offsetLeft + element.offsetWidth > left &&
        element.offsetTop < bottom &&
        element.offsetTop + element.offsetHeight > top
      )
        boardSelection.add(element.dataset.id);
    });
    updateSelection();
    return;
  }
...
document.addEventListener('click', (event) => {
  ...
  if (source && list.contains(source)) {
    ...
    if (selected)
      announce(
        'واژه انتخاب شد. روی صفحه کلیک کن یا با Tab به صفحه برو و Enter بزن.'
      );
...
document.addEventListener('keydown', (event) => {
  ...
  if (
    ...
    return;
  if (element && !boardSelection.has(element.dataset.id))
    boardSelection = new Set([element.dataset.id]);
  ...
  if (event.key === 'Delete' || event.key === 'Backspace') {
    ...
    render();
    board.focus();
    announce('واژه‌های انتخاب‌شده به فهرست برگشتند.');
  } else if (
...
document.querySelector('#reset').addEventListener('click', () => {
  ...
  save();
  render();
  announce('صفحه پاک شد. دوباره شروع کن.');
});
new ResizeObserver(() => {
  applyCamera();
  if (!drag) render();
