...
The grid has equal spacing and no axes, coordinates, or marked origin. The Persian font is loaded from Google Fonts with a local Tahoma fallback. On phones, use two fingers to pinch in or out to zoom. Rectangle selection is disabled on touch-capable devices; individual words can still be dragged.
