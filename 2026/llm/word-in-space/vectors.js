...
for (const word of words) {
  ...
  if (
    !positions[word.id] &&
    !(oldPoint && Number.isFinite(oldPoint.x) && Number.isFinite(oldPoint.y))
  )
    continue;
  ...
  if (!positions[word.id]) {
    positions[word.id] = {
      x:
        Math.max(0, Math.min(1, oldPoint.x)) *
        Math.max(0, board.clientWidth - tile.offsetWidth),
      y:
        Math.max(0, Math.min(1, oldPoint.y)) *
        Math.max(0, board.clientHeight - tile.offsetHeight),
    };
  }
...
function applyCamera() {
  ...
  const major = gridUnit * 5 * camera.scale,
    minor = gridUnit * camera.scale;
...
function fit() {
  ...
  const right = Math.max(
    ...[...tiles].map(([id, tile]) => positions[id].x + tile.offsetWidth + 120)
  );
  const bottom = Math.max(
    ...[...tiles].map(([id, tile]) => positions[id].y + tile.offsetHeight)
  );
  camera.scale = Math.min(
    ...
    Math.max(
      0.05,
      Math.min(
        (board.clientWidth - 64) / (right - left),
        (board.clientHeight - 80) / (bottom - top)
      )
    )
  );
...
window.addEventListener('pointermove', (event) => {
  ...
  if (
    !suppressClick &&
    Math.hypot(point.x - gesture.x, point.y - gesture.y) < 5
  )
    return;
  suppressClick = true;
  ...
  camera.scale =
    gesture.count > 1
      ? Math.max(
          0.05,
          Math.min(4, (start.scale * point.distance) / gesture.distance)
        )
      : start.scale;
...
viewport.addEventListener(
  ...
  (event) => {
    ...
    if (event.ctrlKey) {
      ...
      const next = Math.max(
        0.05,
        Math.min(4, camera.scale * Math.exp(-event.deltaY * 0.01))
      );
