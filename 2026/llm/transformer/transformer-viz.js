(() => {
  ...
  const sentence =
    document.body.dataset.mode === 'decoder'
  ...
  const animate = (duration, onFrame) =>
    new Promise((resolve) => {
      if (reducedMotion) {
        onFrame(1);
        resolve();
  ...
  class EncoderVisualization {
    constructor() {
      ...
      this.base = this.tokens.map((token) => ({
        ...this.baseByWord[token.word],
      }));
      ...
      this.layerCount = 8;
      this.semanticAnchorWords = new Set(
        isPersian
          ? [
              'گورخر',
              'شکار',
    ...
    trajectory(index, layer) {
      ...
      const xNoise =
        Math.sin((index + 1) * 17.3 + layer * 11.7) * 56 * remaining;
      const yNoise =
        Math.cos((index + 1) * 9.1 + layer * 15.4) * 46 * remaining;
      return {
    ...
    setMeaningStage(enabled) {
      this.vocabElements.forEach(({ group, word }) => {
        if (this.semanticAnchorWords.has(word))
    ...
    async transition(state) {
      ...
      if (state === 2) {
        ...
        await sleep(180);
        this.vocabElements.forEach(({ group }) =>
      ...
      if (state === 12) {
        ...
        this.activeElements.forEach(({ dot }, index) => {
          dot.setAttribute(
            'r',
            index === animalIndex || index === objectIndex ? '11' : '8'
          );
        });
        await sleep(500);
      }
  ...
  class DecoderVisualization {
    constructor() {
      ...
      this.base = this.tokens.map((token) => ({
        ...this.embeddingByWord[token.word],
      }));
      this.positionOffsets = isPersian
        ? this.tokens.map((_, index) => ({
            x: Math.round(Math.cos(index * 2.13) * (22 + (index % 3) * 7)),
      ...
      this.contextControls = isPersian
        ? this.contextTargets.map((target, index) => ({
            x:
              (this.positioned[index].x + target.x) / 2 +
              Math.sin(index * 2.4) * 80,
            y:
              (this.positioned[index].y + target.y) / 2 +
              Math.cos(index * 1.8) * 70,
          }))
        : [
            { x: 210, y: 320 },
            { x: 330, y: 96 },
      ...
      this.predictionControls = isPersian
        ? this.predictionTargets.map((target, index) => ({
            x:
              (this.contextTargets[index].x + target.x) / 2 +
              Math.cos(index * 1.7) * 75,
            y:
              (this.contextTargets[index].y + target.y) / 2 +
      ...
      this.predictionControls = this.predictionControls.slice(
        0,
        this.nextTokens.length
      );

      this.contextLayerCount = 4;
      this.predictionLayerCount = 4;
      this.contextAnchorWords = new Set(
        isPersian
          ? [
              'گورخر',
              'شکار',
    ...
    contextPoint(index, layer) {
      ...
      return {
        x:
          point.x + Math.sin((index + 2) * 13.4 + layer * 9.7) * 48 * remaining,
        y:
          point.y + Math.cos((index + 1) * 8.8 + layer * 14.1) * 42 * remaining,
      };
    }
    ...
    predictionPoint(index, layer) {
      ...
      return {
        x:
          point.x +
          Math.cos((index + 1) * 16.2 + layer * 12.5) * 52 * remaining,
        y:
          point.y + Math.sin((index + 3) * 7.6 + layer * 10.8) * 46 * remaining,
      };
    }
    ...
    setContextExamples(enabled) {
      this.vocabElements.forEach(({ group, word }) => {
        if (this.contextAnchorWords.has(word))
          group.classList.toggle('hidden', !enabled);
      });
    }
    ...
    async moveToLayer(overallLayer) {
      const from = this.renderedPositions.map((point) => ({ ...point }));
      const to = this.tokens.map((_, index) =>
        this.trajectory(index, overallLayer)
      );
      await animate(layerMoveDuration, (progress) => {
        this.renderedPositions = from.map((point, index) => ({
          x: point.x + (to[index].x - point.x) * progress,
