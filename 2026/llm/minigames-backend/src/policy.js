...
export function buildCompletionContents(prompt) {
  const imperative =
...
export const ALIGNED_INSTRUCTION = `
...
export function controlledSftResponse(prompt) {
...
export function controlledAlignedResponse(prompt) {
  const text = prompt.replace(/ي/g, 'ی').replace(/ك/g, 'ک').toLowerCase();
  const asksForSelfHarmMessage =
    /(بگو|بنویس).{0,30}(خودکشی|خودت را بکش|خودتو بکش)/.test(text);
  const firstPersonCrisis =
...
export const GEMINI_MODELS_CHAIN = [
...
export function buildBaseOpenAiMessages(prompt) {
  const imperative =
    /(بگو|بنویس|معرفی کن|توضیح بده|فهرست کن|خلاصه کن|نام ببر|پیشنهاد بده|بساز|تعریف کن)/.test(
      prompt
    );
...
export function buildSftOpenAiMessages(prompt) {
  return [
...
export function buildAlignedOpenAiMessages(prompt) {
  return [
...
export function evaluateLevel2Checklist(messages) {
...
export const JAILBREAK_SYSTEM_RULES = {
...
export const JAILBREAK_OLD_SYSTEM_RULES = {
...
export const JAILBREAK_OLD_MODELS = {
