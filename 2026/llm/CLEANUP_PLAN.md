...
| `contexual-embedding/`       | Mouse-only drag, fixed 100vh, inaccessible modal, remote font                               | **Complete:** local typography, responsive viewport, keyboard placement/movement/deletion, focus-visible tokens, and labelled completion dialog; typo URL preserved.                                              |
| `dimensionality-reduction/`  | CDN-only Three.js, random non-reproducible data, English UI, broken 1D navigation           | **Complete:** seeded data, Persian UI/navigation copy, hidden-tab animation pause, touch-sized controls, and reduced-motion-compatible transitions.                                                               |
...
| `model-lab/`                 | Duplicated chat client, stale URL, dead local helpers                                       | **Complete:** shared client/shell, retained per-tab histories, explicit local/Worker source status, and accessible touch-sized model tabs.                                                                        |
...
| `word-embedding/`            | Large monolith, inline handlers, duplicated Three code, unsafe storage                      | **Complete:** local typography, visible keyboard focus, reduced-motion support, and existing hash/state compatibility retained.                                                                                   |
| `wordembedding/`             | Older dense overlap, custom pointer/touch code                                              | **Complete:** compatibility page retains its unique arithmetic while adding visible focus and reduced-motion support.                                                                                             |
